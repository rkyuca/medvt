...
from actor_action.utils.wandb_utils import init_or_resume_wandb_run, get_viz_multicls_img
...
def get_args_parser():
    ...
    # Save Paths
    parser.add_argument('--output_dir', default='/local/riemann1/home/rezaul/medvt/swin_medvt_actor_action/train/',
                             help='path where to save, empty for no saving')
    parser.add_argument('--experiment_name', default='medvt_a2d_exp_{params_summary}')
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    use_wandb: bool = False, vis_freq: int = 1000):
    ...
    for samples, vid_cls, targets, vid_id, gt_index in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if use_wandb:
            wandb_dict = {'loss': loss_value,
                          'lr': optimizer.param_groups[0]["lr"], 'epoch': epoch}
            viz_img = get_viz_multicls_img(
                samples.tensors, targets, outputs, inverse_norm_transform)
            if i_iter % vis_freq == 0:
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(
                ...
                use_wandb=args.use_wandb, vis_freq=args.vis_freq)
        if args.use_lr_warmup:
