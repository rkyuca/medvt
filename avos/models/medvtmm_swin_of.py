...
class MaskHeadSmallConvBack(nn.Module):
    """
    Simple convolutional head, using group norm.
    Upsampling is done using a FPN approach
    """

    def __init__(self, dim, context_dim, relu_before=True):
        super().__init__()
...
class MaskHeadSmallConv(nn.Module):
    ...
    def __init__(self, dim, context_dim, relu_before=True):
        super().__init__()
        print('Creating FPN-> dim:%d context_dim:%d relu_before:%s' % (dim, context_dim, str(relu_before)))
        inter_dims = [dim, context_dim, context_dim, context_dim, context_dim, context_dim]
        self.lay1 = torch.nn.Conv2d(dim, dim, 3, padding=1)
        self.gn1 = torch.nn.GroupNorm(8, dim)
        self.lay2 = torch.nn.Conv2d(dim, inter_dims[1], 3, padding=1)
...
class Transformer(nn.Module):
    ...
    def __init__(self, num_frames, backbone_dims, d_model=384, nhead=8,
        ...
        self.flow_proj_modules = nn.ModuleList()

        self.use_encoder = sum(self.num_encoder_layers) > 0
        self.mm_fuse_type = mm_fuse_type
        self.num_encoder_stages = len(self.num_encoder_layers)

        self.use_decoder = num_decoder_layers > 0
        self.qgen_dec_layers=qgen_dec_layers if qgen_dec_layers is not None else 0
        ...
        if self.use_flow:
            for backbone_dim in backbone_dims:
                self.flow_proj_modules.append(nn.Conv2d(backbone_dim, d_model, kernel_size=1))
            if self.mm_fuse_type is not None:
                assert self.mm_fuse_type in ['attn', 'cat','attn_cat']
                if self.mm_fuse_type in ['attn', 'attn_cat']:
                    self.mm_context_encoder = MMContextEncoderBiDirCrossAttn(vis_dims=[d_model, d_model, d_model, d_model], ctx_dims=[d_model, d_model, d_model, d_model], stages=fuse_stages)
                if self.mm_fuse_type in ['cat', 'attn_cat']:
                    self.mm_fuse_proj = nn.ModuleList()
                    for i in range(fuse_stages):
                        self.mm_fuse_proj.append(nn.Conv2d(d_model*2, d_model, kernel_size=1))
        ...
        if num_decoder_layers > 0:
            if decoder_bbox_head == 'tube':
                ...
                self.qf_norm = nn.LayerNorm(bbox_nhead)
            else:
            ...
            if self.use_flow and self.qgen_dec_layers>0:
                self.qgen_decoder = TransformerDecoderUniModal(decoder_layer, self.qgen_dec_layers, decoder_norm,
        ...
        #else:
        self.fpn = MaskHeadSmallConv(dim=d_model, context_dim=d_model, relu_before=fpn_relu_before)
    ...
    def forward(self, features, flow_feats, pos_list, batch_size):
        # features as TCHW
        # import ipdb; ipdb.set_trace()
        bt = features[-1].tensors.shape[0]
        bs_f = bt // self.num_frames
        # ##########################################################
        # ##### < Feature projection to transformer dims > #########
        ...
        # project all flow backbone features to transformer dim
        # import ipdb;ipdb.set_trace()
        if flow_feats is not None:
            for i in range(len(flow_feats)):
                src, mask = flow_feats[i].decompose()
                assert mask is not None
                src_proj = self.flow_proj_modules[i](src)
                flow_feats[i] = NestedTensor(src_proj, mask)

        # import ipdb;ipdb.set_trace()
        if flow_feats is not None and self.mm_fuse_type is not None:
            if self.mm_fuse_type == 'attn':
                features, flow_feats = self.mm_context_encoder(features, flow_feats, pos_list, pos_list)
            elif  self.mm_fuse_type == 'attn_cat':
                features, flow_feats = self.mm_context_encoder(features, flow_feats, pos_list, pos_list)
                for j in range(self.fuse_stages):
                    i = -1-j
                    src = self.mm_fuse_proj[i]( torch.cat([features[i].tensors, flow_feats[i].tensors],dim=1))
                    features[i] = NestedTensor(src, features[i].mask)
            else:
                for j in range(self.fuse_stages):
                    i = -1-j
                    src = self.mm_fuse_proj[i]( torch.cat([features[i].tensors, flow_feats[i].tensors],dim=1))
        ...
        if self.use_decoder:
            ...
            if use_fpn_before_query_decoder :
                ms_feats = list(reversed(self.fpn(deep_feature, fpn_features)))
                hr_feat = ms_feats[0]
                dec_features = []
                dec_mask_list = []
                pos_embed_list = []
                ...
                for i in range(self.query_decoder_scales):
                    fi = ms_feats[self.query_decoder_scales-1-i]
                    ni, ci, hi, wi = fi.shape
                    fi = fi.reshape(bs_f, self.num_frames, ci, hi, wi).permute(0, 2, 1, 3, 4).flatten(-2).flatten(
                        2).permute(2, 0,
                                   1)
                    dec_mask_i = features[self.query_decoder_scales-1 - i].mask.reshape(bs_f, self.num_frames, hi * wi).flatten(1)
                    pe = pos_list[self.query_decoder_scales-1 - i].permute(0, 2, 1, 3, 4).flatten(-2).flatten(2).permute(2, 0, 1)
                    dec_features.append(fi)
                    pos_embed_list.append(pe)
                    size_list.append((hi, wi))
                    dec_mask_list.append(dec_mask_i)
                if flow_feats is not None and self.qgen_dec_layers>0:
                    dec_ffeatures = []
                    dec_fmask_list = []
                    dec_fpos_list = []
                    dec_fsize_list = []
                    #import ipdb;ipdb.set_trace()
                    ffi, fmask = flow_feats[-1].decompose()
                    ni, ci, hi, wi = ffi.shape
                    ffi = ffi.reshape(bs_f, self.num_frames, ci, hi, wi).permute(0, 2, 1, 3, 4).flatten(-2).flatten(
                        2).permute(2, 0,1)
                    fmi = fmask.reshape(bs_f, self.num_frames, hi * wi).flatten(1)
                    fpos = pos_list[-1].permute(0, 2, 1, 3, 4).flatten(-2).flatten(2).permute(2, 0, 1)
                    dec_ffeatures.append(ffi)
                    dec_fmask_list.append(fmi)
                    dec_fpos_list.append(fpos)
                    dec_fsize_list.append((hi, wi))
                ...
                tgt = torch.ones_like(query_embed)*1e-6  # * functools.reduce(lambda x,y:x*y, features[0].tensors.shape) #1e-7  #(1.0/(size_list[0][0]*size_list[0][1]))
                # import ipdb;ipdb.set_trace()
                if flow_feats is not None and self.qgen_dec_layers>0:
                    # import ipdb;ipdb.set_trace()
                    hs = self.qgen_decoder(tgt, dec_ffeatures, memory_key_padding_mask=dec_fmask_list,
                                  pos=dec_fpos_list, query_pos=query_embed, size_list=dec_fsize_list)
                    # import ipdb;ipdb.set_trace()
                    tgt = hs[-1]
                ...
                if self.decoder_bbox_head == 'tube':
                    ...
                    # import ipdb;ipdb.set_trace()
                    obj_attn_masks = torch.einsum('bqc,btchw->btqhw', hs[-1], hr_feat.unsqueeze(0)).squeeze(0)
                    # b,q,h,w = qf.shape
                    obj_attn_masks = self.qf_norm(obj_attn_masks.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
                else:
...
class MMContextEncoderBiDirCrossAttn(nn.Module):
    def __init__(self, vis_dims=[384, 384, 384, 384], ctx_dims=[384, 384, 384, 384], stages=2):
        super(MMContextEncoderBiDirCrossAttn, self).__init__()
        self.num_stages = stages #4 # len(vis_dims)
        ...
        for i in range(self.num_stages):
            self.attn_modules.append(
                BidirectionalCrossAttention(
                    dim=vis_dims[-1-i],  # video channels
                    heads=1,
                    context_dim=ctx_dims[-1-i]  # flow channels
                )
            )
        self._reset_parameters()
    ...
    def forward(self, vis_features, ctx_features, vis_pos_list, ctx_pos_list):
        """
        vis_features = [ (f[bt, C, h, w],m[bt,h,w]), ..]
        ctx_features = [ (f[bt, C, h, w],m[bt,h,w]), ..]
        vis_pos_list = [ p[b,t,c,h,w] ]
        """
        # import ipdb;ipdb.set_trace()
        for j in range(self.num_stages):
            ...
            video_feat, video_mask = vis_features[i].decompose()  # BT x C x H x W
            ctx_feat, ctx_mask = ctx_features[i].decompose()  # .clone().detach()  # BT x C
            b, t, c, h, w = vis_pos_list[i].shape

            if self.attn_type == 'thw':
                video_feat = video_feat.reshape(b, t, c, h, w).permute(0, 2, 1, 3, 4).flatten(
                    2).permute(0, 2, 1)
                video_pos = vis_pos_list[i].clone().detach().permute(0, 2, 1, 3, 4).flatten(
                    2).permute(0, 2, 1).to(video_feat.device)
                video_mask = torch.ones((b, t * h * w)).bool().to(video_feat.device)
                # import ipdb;ipdb.set_trace()
                ctx_feat = ctx_feat.reshape(b, t, c, h, w).permute(0, 2, 1, 3, 4).flatten(
                    2).permute(0, 2, 1)
                ctx_pos = ctx_pos_list[i].clone().detach().permute(0, 2, 1, 3, 4).flatten(
                    2).permute(0, 2, 1).to(video_feat.device)
                ctx_mask = torch.ones((b, t * h * w)).bool().to(video_feat.device)
                ...
                video_pos = vis_pos_list[i].clone().detach().flatten(0, 1).flatten(2).permute(0, 2, 1).to(video_feat.device)
                video_mask = torch.ones((b * t, h * w)).bool().to(video_feat.device)
                # import ipdb;ipdb.set_trace()
                ctx_feat = ctx_feat.flatten(2).permute(0, 2, 1)
                ctx_pos = ctx_pos_list[i].clone().detach().flatten(0, 1).flatten(2).permute(0, 2, 1).to(video_feat.device)
                ctx_mask = torch.ones((b * t, h * w)).bool().to(video_feat.device)
            ...
            #  import ipdb;ipdb.set_trace()
            # import ipdb;ipdb.set_trace()
            video_out, ctx_out = joint_cross_attn(
            ...
            # import ipdb; ipdb.set_trace()
            if self.attn_type == 'thw':
                video_feat = video_out.permute(0, 2, 1).reshape(b, c, t, h, w).permute(0, 2, 1, 3,4).flatten(
                    0, 1)
                ctx_feat = ctx_out.flatten(0, 1)
                vis_features[i] = video_feat
                ctx_features[i] = ctx_feat
            else:
                video_feat = NestedTensor(video_out.permute(0, 2, 1).reshape(b * t, c, h, w), vis_features[i].mask)
                ctx_feat =  NestedTensor(ctx_out.permute(0, 2, 1).reshape(b * t, c, h, w), ctx_features[i].mask)
                vis_features[i] = video_feat
                ctx_features[i] = ctx_feat
            # attended output should have the same shape as input
            # assert video_out.shape == video.shape
            # assert audio_out.shape == audio.shape
        return vis_features, ctx_features
...
class TransformerDecoderUniModal(nn.Module):
...
class VOS_SwinMEDVT(nn.Module):
    def __init__(self, args, backbone, flow_backbone, backbone_dims, hidden_dim, transformer, num_frames, temporal_strides=[1], n_class=1):
        super().__init__()
        # import ipdb; ipdb.set_trace()
    ...
    def forward(self, samples: NestedTensor, **kwargs):
        # import ipdb;ipdb.set_trace()
        if self.training:
            return self._forward_one_samples(samples, **kwargs)
        else:
            return self.forward_inference(samples, **kwargs)

    def forward_inference(self, samples: NestedTensor, **kwargs):
        # import ipdb;ipdb.set_trace()
        ...
        with torch.no_grad():
            for stride, samples_ in samples.items():
                all_outs.append(self._forward_one_samples(samples_, **kwargs))
            all_outs = torch.stack([a['pred_masks'] for a in all_outs], dim=0)
            mean_outs = all_outs.mean(0)
        return {'pred_masks': mean_outs}

    def _forward_backbone1(self, samples: NestedTensor, **kwargs):
    ...
    def _forward_backbone2(self, samples: NestedTensor, of):
        if self.flow_backbone is None:
            return None 
        if self.backbone_name == 'resnet101' or self.backbone_name == 'resnet50':
            features, pos_list = self.backbone(of)  # ## check warnings on floor_divide
            # import ipdb; ipdb.set_trace()
            T, _, H, W = features[0].tensors.shape
            batch_size = T // self.num_frames
        elif self.backbone_name == 'pvt':
            flow_feats = self.flow_backbone(of)
            for i in range(len(flow_feats)):
                x = flow_feats[i]
                m = samples.mask
                assert m is not None
            ...
            flow_feats = self.flow_backbone(of.permute(1,0,2,3).unsqueeze(0))
            for i in range(len(flow_feats)):
                # check flow feature and image feature shapes and interpolate if necessary
                x = flow_feats[i].permute(0, 2, 1, 3, 4).flatten(0, 1)
                m = samples.mask
                assert m is not None
                mask = F.interpolate(m[None].float(), size=x.shape[-2:]).to(torch.bool)[0]
                x = NestedTensor(x, mask)
    ...
    def _forward_backbones(self, samples: NestedTensor, **kwargs):
        of = None
        flow_feats = None
        if 'flows' in kwargs and kwargs['flows'] is not None:
            of = kwargs['flows']
        features, pos_list = self._forward_backbone1(samples, **kwargs)
        if of is not None:
            flow_feats = self._forward_backbone2(samples, of)
        return features, pos_list, flow_feats

    def _forward_one_samples(self, samples: NestedTensor, **kwargs):
        # import ipdb;ipdb.set_trace()
        """
        if not isinstance(samples, NestedTensor):
            samples = nested_tensor_from_tensor_list(samples)
        ...
        if of is not None:
            flow_feats = self._forward_backbone2(samples, of)
        """
        features, pos_list, flow_feats = self._forward_backbones(samples, **kwargs)
        batch_size = 1 #TODO  fix it later to use more that 1 batch
        # #################################################################
        # ###### Backbone feature extraction done #########################
        # #################################################################
        # If flow available, fuse flow feats with image features after the backbone
        # #################################################################
        # import ipdb; ipdb.set_trace()
        ...
        if self.transformer is None:
            ...
            # import ipdb; ipdb.set_trace()
            seg_feats = self.transformer(features, flow_feats, pos_list, batch_size)
...
class VOS_SwinMEDVTLPROP(VOS_SwinMEDVT):
    def __init__(self, args, backbone, flow_backbone, backbone_dims, hidden_dim, transformer, num_frames,
                 pretrain_settings={}, lprop_mode=None, temporal_strides=[1], feat_loc=None,
                 stacked=1, n_class=1):
        # import ipdb; ipdb.set_trace()
        super().__init__(args, backbone,flow_backbone, backbone_dims, hidden_dim, transformer, num_frames, n_class=n_class)
    ...
    def _forward_one_samples(self, samples: NestedTensor, **kwargs):
        ...
        if 'flows' in kwargs and kwargs['flows'] is not None:
            of = kwargs['flows']
            # of = NestedTensor(of, samples.mask)
...
def initialize_pvt_weights(pvt_model, pvt_weights_path):
    pvt_model_dict = pvt_model.state_dict()
    pretrained_state_dicts = torch.load(pvt_weights_path)
    ...
    pvt_model.load_state_dict(pvt_model_dict)
    print(f'==> Load pvt-v2-b5 parameters pretrained on ImageNet from {pvt_weights_path}')
    return pvt_model
...
def build_model_medvt_swinbackbone_without_criterion(args):
    ...
    if args.backbone == 'swinS':
        ...
        backbone = pvt_v2_b5()
        backbone = initialize_pvt_weights(backbone, args.pvt_weights_path)
        backbone_dims = [64, 128, 320, 512]
    else:
        raise ValueError('backbone: %s not implemented!' % args.backbone)

    flow_backbone = None 
    if hasattr(args, 'use_flow') and args.use_flow==1:
        print('using flow backbone ...')
        if args.backbone == 'swinS':
            flow_backbone = build_swin_s_backbone(args.is_train, args.swin_s_pretrained_path)
        elif args.backbone == 'swinB':
            flow_backbone = build_swin_b_backbone(args.is_train, args.swin_b_pretrained_path)
        elif args.backbone == 'pvt':
            from .pvt import pvt_v2_b5
            flow_backbone = pvt_v2_b5()
            flow_backbone = initialize_pvt_weights(flow_backbone, args.pvt_weights_path)
        else:
            raise ValueError('backbone: %s not implemented!' % args.backbone)
    ...
    transformer = Transformer(
        ...
        mm_fuse_type = args.mm_fuse_type if hasattr(args,'mm_fuse_type') else None,
        qgen_dec_layers=args.qgen_dec_layers if hasattr(args, 'qgen_dec_layers') else None,
        fpn_relu_before = args.fpn_relu_before if hasattr(args, 'fpn_relu_before') else True,
        fuse_stages = args.fuse_stages if hasattr(args,'fuse_stages') else 4,
        use_flow = args.use_flow if hasattr(args,'use_flow') else 0,
    )
    ...
    if args.lprop_mode > 0:
        ...
        model = VOS_SwinMEDVTLPROP(args, backbone, flow_backbone, backbone_dims=backbone_dims, hidden_dim=args.hidden_dim,
        ...
        model = VOS_SwinMEDVT(args, backbone, flow_backbone, backbone_dims=backbone_dims, hidden_dim=args.hidden_dim,
