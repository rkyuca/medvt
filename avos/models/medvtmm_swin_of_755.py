...
class Transformer(nn.Module):
    ...
    def __init__(self, num_frames, backbone_dims, d_model=384, nhead=8,
        ...
        self.qgen_dec_layers=qgen_dec_layers
        ...
        if self.mm_fuse_type=='attn':
            self.mm_context_encoder = MMContextEncoderBiDirCrossAttn(vis_dims=[d_model, d_model, d_model, d_model], ctx_dims=[d_model, d_model, d_model, d_model])
        else:
    ...
    def forward(self, features, flow_feats, pos_list, batch_size):
        ...
        if self.use_decoder:
            ...
            if use_fpn_before_query_decoder :
                ...
                tgt = torch.ones_like(query_embed)  #* functools.reduce(lambda x,y:x*y, features[0].tensors.shape) #1e-7  #(1.0/(size_list[0][0]*size_list[0][1]))
                # import ipdb;ipdb.set_trace()
                if flow_feats is not None and self.qgen_dec_layers>0:
                    # import ipdb;ipdb.set_trace()
...
class MMContextEncoderBiDirCrossAttn(nn.Module):
    def __init__(self, vis_dims=[384, 384, 384, 384], ctx_dims=[384, 384, 384, 384]):
        super(MMContextEncoderBiDirCrossAttn, self).__init__()
        self.num_stages = 4 # len(vis_dims)
        ...
        for i in range(self.num_stages):
            self.attn_modules.append(
                BidirectionalCrossAttention(
                    dim=vis_dims[i],  # video channels
                    heads=1,
                    context_dim=ctx_dims[i]  # flow channels
                )
            )
        self._reset_parameters()
        ...
        for j in range(self.num_stages):
            ...
            if self.attn_type == 'thw':
                video_feat = video_out.permute(0, 2, 1).reshape(b, c, t, h, w).permute(0, 2, 1, 3,4).flatten(
                    0, 1)
                audio_feat = ctx_out.flatten(0, 1)
                vis_features[i] = video_feat
                ctx_features[i] = audio_feat
            else:
...
def build_model_medvt_swinbackbone_without_criterion(args):
    ...
    transformer = Transformer(
        ...
        mm_fuse_type = args.mm_fuse_type,
        qgen_dec_layers=args.qgen_dec_layers
    )
