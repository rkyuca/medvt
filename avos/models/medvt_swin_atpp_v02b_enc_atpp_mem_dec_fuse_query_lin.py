...
class Transformer(nn.Module):
    ...
    def __init__(self, num_frames, backbone_dims, d_model=384, nhead=8,
        ...
        self.global_query_memory = global_query_memory
        ...
        if self.dec_feat_atpp:
            self.dec_atpp = DecATPP(input_dim=d_model+bbox_nhead, model_dim=d_model+bbox_nhead, output_dim=d_model+bbox_nhead, num_branches=dec_feat_atpp_branches,
    ...
    def forward(self, features, pos_list, batch_size):
        ...
        # trans_stage_outputs.append(features[-1].tensors.permute(1, 0, 2, 3).flatten(1).mean(1).unsqueeze(0))
        ...
        if self.use_decoder:
            if self.decoder_multiscale:
                ...
                if self.decoder_bbox_head == 'tube':
                    ...
                    if self.global_query_memory:
                        tgt = self.query.weight.unsqueeze(1)
                    else:
                    ...
                    if self.global_query_memory:
                        tgt = self.query.weight.unsqueeze(1).repeat(self.num_frames // tq, bs_f, 1)
                    else:
                        tgt = torch.zeros_like(query_embed)
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, num_encoder_layers, nhead, dim_feedforward, d_model, dropout, activation, normalize_before,
                 use_cross_layers, cross_pos, num_classes=1, enc_atpp_branch=1, enc_atpp_res=False):
        super().__init__()
        # ######################################
        # import ipdb; ipdb.set_trace()
...
def build_model_medvt_swinbackbone_without_criterion(args):
    ...
    transformer = Transformer(
        ...
        global_query_memory=args.global_query_memory,
