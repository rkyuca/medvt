...
class Transformer(nn.Module):
    ...
    def forward(self, features, pos_list, batch_size):
        ...
        for i in range(len(features)):
            ...
            trans_stage_outputs.append(src_proj.clone())
        ...
        if self.use_decoder:
            ...
            if use_fpn_before_query_decoder:
                ...
                obj_attn_masks = []
                query_feat = ms_feats[-1] if self.query_decoder_scales > 1 else ms_feats[0]
                query_feat_mask = features[0].mask if self.query_decoder_scales > 1 else features[-1].mask
                # import ipdb; ipdb.set_trace()
                for i in range(self.num_frames):
                    t2, c2, h2, w2 = query_feat.shape
                    hs_f = hs[-1][:, i * n_f:(i + 1) * n_f, :]
                    memory_f = query_feat[i, :, :, :].reshape(batch_size, c2, h2, w2)
                    mask_f = query_feat_mask[i, :, :].reshape(batch_size, h2, w2)
                ...
                if self.query_decoder_scales <= 1:
                    obj_attn_masks = F.interpolate(obj_attn_masks, hr_feat.shape[-2:])
                if self.decoder_attn_fuse is not None and self.decoder_attn_fuse == 'add':
