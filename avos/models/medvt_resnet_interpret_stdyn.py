...
class Transformer(nn.Module):
    ...
    def forward(self, s_h, s_w, s_h2, s_w2, t_f, src, mask, query_embed, pos_embed, src2, mask2, pos2, features,
        ...
        for i in range(len(features)):
            trans_stage_outputs.append(features[i].tensors.clone())
        ...
        memory, memory2, enc_intermediates = self.encoder(s_h, s_w, s_h2, s_w2, t_f, src, src_key_padding_mask=mask,
                                                          pos=pos_embed,
        ...
        # fpn_intermediates = [fi.clone() for fi in ms_feats] + [hr_feat.clone()]
        ...
        # import ipdb; ipdb.set_trace()
        dec_intermediates = [hs[i].squeeze(1).unsqueeze(2).unsqueeze(2) for i in range(len(hs))]
        trans_stage_outputs = trans_stage_outputs + dec_intermediates
        return hs.transpose(1, 2), hr_feat, trans_stage_outputs
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, s_h, s_w, s_h2, s_w2, t_f, src,
        ...
        if self.cross_pos == -1 or self.cross_pos == 2:
            ...
            # <Intermediates >
            # import ipdb;ipdb.set_trace()
            interm = output2.permute(1, 2, 0).view(1, 384, t_f, s_h2 * s_w2)
            interm = interm.permute(0, 2, 1, 3).reshape(1, t_f, 384, s_h2, s_w2).flatten(0, 1)
        ...
        for i in range(6):
            ...
            # <Intermediates >
            # import ipdb;ipdb.set_trace()
            interm = output.permute(1, 2, 0).view(1, 384, t_f, s_h * s_w)
            interm = interm.permute(0, 2, 1, 3).reshape(1, t_f, 384, s_h, s_w).flatten(0, 1)
            all_intermediates.append(interm)
            # </ intermediates>
            # #########################################
        ...
        return output, output2, all_intermediates
...
class MEDVT(nn.Module):
    ...
    def forward_inference(self, samples: NestedTensor, clip_tag=None):
        return self._forward_one_samples(samples, clip_tag=clip_tag)
...
class MEDVT_LPROP(MEDVT):
    ...
    def _forward_one_samples(self, samples: NestedTensor, clip_tag=None, stages=[]):
        # import ipdb;ipdb.set_trace()
        stage_outputs = []
        if not isinstance(samples, NestedTensor):
            samples = nested_tensor_from_tensor_list(samples)
        ...
        hs, hr_feat, trans_stages = self.vistr.transformer(s_h, s_w, s_h2, s_w2, self.vistr.num_frames, src_proj, mask,
                                                           self.vistr.query_embed.weight, pos, src_proj_2, mask_2,
                                                           pos_2, features,
                                                           pos_list)
        # import ipdb;ipdb.set_trace()
        ...
        # import ipdb;ipdb.set_trace()
        seg_feats = torch.cat([hr_feat, obj_attn_masks], dim=1)
        # stage_outputs.append(seg_feats.clone())
        ...
        if hasattr(self,'aux_loss') and self.aux_loss:
            out['aux_pred_masks'] = outputs_seg_masks_medvt
        return out
