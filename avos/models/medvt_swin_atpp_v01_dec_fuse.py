...
class Transformer(nn.Module):
    ...
    def __init__(self, num_frames, backbone_dims, d_model=384, nhead=8,
        ...
        if self.use_atpp:
            ...
            self.dec_atpp = DecATPP(input_dim=384, model_dim=128, output_dim=8, num_branches=3)
    ...
    def forward(self, features, pos_list, batch_size):
        ...
        if self.use_decoder:
            if self.decoder_multiscale:
                ...
                #import ipdb; ipdb.set_trace()
                if self.use_atpp:
                    # atpp_in = hr_feat.permute(1,0,2,3).unsqueeze(0)
                    dec_atpp = self.dec_atpp(hr_feat.permute(1,0,2,3).unsqueeze(0)).squeeze(0).permute(1,0,2,3)
                    #dec_atpp =dec_atpp.squeeze(0).permute(1,0,2,3)
                    hr_feat = torch.cat([hr_feat,dec_atpp], dim=1)
                if self.decoder_attn_fuse is not None and self.decoder_attn_fuse == 'add':
    ...
    def forward(self, features, src_key_padding_masks, pos_embeds, sizes, masks=None, pred_mems=None):
        ...
        for i in range(self.num_stages):
            ...
            for j in range(self.num_layers[i]):
                output = self.layers[i][j](output, src_mask=None, src_key_padding_mask=skp_mask, pos=pos_embed,size=sizes[i])
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        if self.use_atpp:
            from avos.models.atpp import EncATPP
            self.key_atpp= EncATPP(input_dim=384, model_dim=384, output_dim=384, num_branches=2, groups=4, dropout=0.1, use_res=False)
