...
class VOS_SwinMEDVT(nn.Module):
    ...
    def forward_inference(self, samples: NestedTensor):
        ...
        with torch.no_grad():
            ...
            aux_pred = all_outs[0]['aux_pred_masks']
            all_preds = torch.stack([a['pred_masks'] for a in all_outs], dim=0)
            mean_outs = all_preds.mean(0)
        return {'pred_masks': mean_outs, 'aux_pred_masks': aux_pred}
