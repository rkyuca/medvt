    ...
    def forward(self, feature_map_list, audio_features, pos_list, batch_size, num_frames):
        ...
        for i in range(self.num_stages):
            ...
            if self.attn_type=='thw':
                ...
                audio_pos = self.context_pos_embeds[i].weight.unsqueeze(0).repeat(batch_size,1,1).to(video_feat.device)
            ...
            # import ipdb; ipdb.set_trace()
            if self.attn_type=='thw':
                video_feat = video_out.permute(0,2,1).reshape(batch_size,vc,num_frames,vh,vw).permute(0,2,1,3,4).flatten(0,1)
...
class MMContextEncoderTPAVI(nn.Module):
    def __init__(self, config, args, vis_dim=[64, 128, 320, 512], channel=384):
        super(MMContextEncoderTPAVI, self).__init__()
        if not hasattr(args, 'tpavi_stages'):
            tpavi_stages = []
        else:
            tpavi_stages = args.tpavi_stages

        if not hasattr(args, 'tpavi_vv_flag'):
            tpavi_vv_flag = False
        else:
            tpavi_vv_flag = args.tpavi_vv_flag

        if not hasattr(args, 'tpavi_va_flag'):
            tpavi_va_flag = True
        else:
    ...
    def forward(self, visual_features, audio_features, batch_size,num_frames, **kwargs):
        # BT x C x H x W
        #a_fea_list = [None] * 4
        if len(self.tpavi_stages) > 0:
            ...
            for i in self.tpavi_stages:
                tpavi_count = 0
                conv_feat = torch.zeros_like(visual_features[i]).cuda()
                if self.tpavi_vv_flag:
                    conv_feat_vv = self.tpavi_vv(visual_features[i], stage=i)
                    conv_feat += conv_feat_vv
                    tpavi_count += 1
                if self.tpavi_va_flag:
                    conv_feat_va, a_fea = self.tpavi_va(visual_features[i], audio_features[i], stage=i)
                    conv_feat += conv_feat_va
                    tpavi_count += 1
                    audio_features[i] = a_fea.flatten(0,1)
                conv_feat /= tpavi_count
                visual_features[i] = conv_feat  # update features of stage-i which conduct non-local
...
class QueryDecoder(nn.Module):
    def __init__(self, num_queries_per_frame=384, 
        ...
        if num_decoder_layers > 0:
            ...
            # self.context_proj = nn.Linear(in_features=128, out_features=384)
            # self.context_pos_embed = nn.Embedding(num_frames, d_model)            
            ...
            # self.context_proj =   MLP(context_channel, 2048, num_queries_per_frame, 3)
            self.query_initializer = ContextQueryGenerator2(num_layers=3,embed_dim=d_model)
            # self.query_initializer = AttentionQueryGenerator(num_layers=6, query_num=num_queries_per_frame)
    ...
    def forward(self,visual_feats, audio_feat, pos_list, batch_size, num_frames): 
        ...
        query = torch.zeros_like(query_pos).to(query_pos.device)
        context_feat = self.context_proj(audio_feat.clone().detach().to(query_pos.device)).unsqueeze(0) # .permute(1,0,2)
        # context_pos = self.context_pos_embed.weight.repeat(batch_size,1).unsqueeze(0).to(context_feat.device)

        # import ipdb;ipdb.set_trace()
        # query_init_feat = self.query_init0(audio_feat.clone().detach().to(query_pos.device))
        # query_init_feat = query_init_feat.reshape(batch_size*num_frames, self.num_queries_per_frame, self.d_model)
        # query = 0.1*query_init_feat.permute(1,0,2)
        query = self.query_initializer(query, context_feat, query_pos=query_pos_ctx, context_pos=None)
        # import ipdb;ipdb.set_trace()
        # tgt = tgt.permute(1,0,2)
        # tgt = self.query_initializer(query_initi_feat).permute(1,0,2)
        # import ipdb;ipdb.set_trace()
...
class MEDVT(nn.Module):
    def __init__(self, args, channel=384, additional_mode = 'flow'):
        """
        args: 
        channel: 
        additional_mode:
        """    
        super(MEDVT, self).__init__()
        self.d_model = channel
        self.visual_backbone = args.visual_backbone
        ...
        if self.visual_backbone == 'swinB':
            from .swin_transformer_3d import build_swin_b_backbone
            self.encoder_backbone = build_swin_b_backbone()
            self.vis_dim = [256, 512, 1024, 1024]
        else:
            # PVT
            self.encoder_backbone = pvt_v2_b5()
            self.vis_dim =[64, 128, 320, 512]


        if additional_mode == 'flow':
            if self.visual_backbone == 'swinB':
                from .swin_transformer_3d import build_swin_b_backbone
                self.mm_backbone = build_swin_b_backbone()
                self.mm_feat_dims = [256, 512, 1024, 1024]
            else:
                # PVT
                self.mm_backbone = pvt_v2_b5()
                self.mm_feat_dims = [64, 128, 320, 512]
    ...
    def forward(self, samples: NestedTensor, **kwargs):
        import ipdb;ipdb.set_trace()
        audio_feature=kwargs['audio_feature']
        batch_size=kwargs['batch_size']
        num_frames=kwargs['num_frames']
        flows = kwargs['flow']
        # shape of x is (b*t)x 3 x 224, 224
        # Extract backbone features for images
        x = samples.tensors
        _, _, i_h, i_w = x.shape
        visual_features = None
        if self.visual_backbone == 'swinB':
            bt, c, h, w = x.shape
            # x = F.interpolate(x, (int(h * 2), int(w * 2)))
        ...
        # Extract backbone features for flow 
        # TODO 
        ...
        # visual_features = self.fpn1(visual_features[3], list(reversed(visual_features[:3])))[::-1]
 
        ########################
        # SwinB
        # import ipdb;ipdb.set_trace()
        # visual_features, pos_list = self.visual_context_encoder(visual_features)

        # feature_map_list = visual_features
        # Visual feature list done here, can replace fully with MED-VT
        # feature map list [ [BF x 256 x H x W], ... ]
        # use transformer encoders here, and then reshape to frame features back
        # import ipdb;ipdb.set_trace()
        ...
        # Replace below with FPN ms features
        # import ipdb;ipdb.set_trace()
        visual_features = self.fpn(visual_features[3], list(reversed(visual_features[:3])))[::-1]
        # import ipdb;ipdb.set_trace()
        #visual_features, audio_features = self.mm_context_encoder2(visual_features, audio_features, pos_list=pos_list,
        #                                                      batch_size=batch_size, num_frames=num_frames)
        #import ipdb;ipdb.set_trace()
        ...
        # bt, c, h, w = conv4321.shape
        # import ipdb;ipdb.set_trace()
        task_head_in = F.interpolate(task_head_in, (i_h // 2, i_w // 2))
        # Use MS Transformer decoder before task head

        # Replace taskhead with 3D conv
        # import ipdb;ipdb.set_trace()
        pred = self.output_conv(task_head_in)  # BF x 1 x 224 x 224
        # print(pred.shape)
        pred = F.interpolate(pred, (i_h, i_w))
        # pred.shape B*F x 1 x H x W
        # feature map list [(B*F, 256, Hi, Wi )  ]*4
        # a feat list shapes: [ (B x F x 256 )...]*4
