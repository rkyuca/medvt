...
from avos.models.label_propagation_viz import LabelPropagator
...
class MHAttentionMap(nn.Module):
    ...
    def forward(self, q, k, mask=None):
        ...
        # weights1 = F.softmax(weights.flatten(3), dim=-1).view_as(weights)
...
class Transformer(nn.Module):
    ...
    def forward(self, features, pos_list, batch_size, intermediate_dict=None):
        # features as TCHW
        # import ipdb; ipdb.set_trace()
        ...
        for i in range(len(features)):
            ...
            if intermediate_dict is not None:
                intermediate_dict['fe_%d' % i] = src_proj.clone()

        # import ipdb; ipdb.set_trace()
        # trans_stage_outputs.append(features[-1].tensors.clone())  # TxCxHxW

        # trans_stage_outputs.append(features[-1].tensors.flatten(0).unsqueeze(0))
        # trans_stage_outputs.append(
        #    features[-1].tensors.permute(1, 0, 2, 3).flatten(1).permute(1, 0).mean(dim=0, keepdim=True))
        # trans_stage_outputs.append(features[-1].tensors.permute(1,0,2,3).flatten(1).permute(1,0).std(dim=0,keepdim=True))
        # trans_stage_outputs.append(features[-1].tensors.permute(1, 0, 2, 3).flatten(1).permute(1, 0).norm(dim=0, keepdim=True))
        ...
        if self.use_encoder:
            ...
            encoder_features, intermediate_dict = self.encoder(features=enc_feat_list,
        ...
        if self.use_decoder:
            if True:
                ...
                # import ipdb; ipdb.set_trace()
                # fpn_intermediates = [ms_feats[j].clone() for j in range(len(ms_feats))]
                # fpn_intermediates = [deep_feature] + [ms_feats[j].clone() for j in range(1,len(ms_feats))]
                # if intermediate_dict is not None:
                #    for i, feats in enumerate(fpn_intermediates):
                #        intermediate_dict['pd_%d' % i] = feats.clone()
                hr_feat = ms_feats[-1]
                dec_features = []
                pos_embed_list = []
                size_list = []
                ...
                #    for i, query_i in enumerate(dec_intermediates):
                #        intermediate_dict['qd_%d' % i] = query_i.clone()
                ...
                # import ipdb;ipdb.set_trace()
                if intermediate_dict is not None:
                    intermediate_dict['qam'] = obj_attn_masks
                # import ipdb; ipdb.set_trace()
                if self.decoder_attn_fuse is not None and self.decoder_attn_fuse == 'add':
                ...
                # import ipdb; ipdb.set_trace()
                if intermediate_dict is not None:
                    intermediate_dict['task_head_input'] = seg_feats.clone()
        else:
            ms_feats = self.fpn(deep_feature, fpn_features)
            hr_feat = ms_feats[-1]
            seg_feats = hr_feat
        return seg_feats, intermediate_dict


class TransformerEncoder(nn.Module):

    def __init__(self, num_encoder_layers, nhead, dim_feedforward, d_model, dropout, activation, normalize_before,
                 use_cross_layers, cross_pos, return_intermediate=True):
        super().__init__()
        # ######################################
        # import ipdb; ipdb.set_trace()
    ...
    def forward(self, features, src_key_padding_masks, pos_embeds, sizes, masks=None, pred_mems=None,
        ...
        for i in range(self.num_stages):
            ...
            if self.use_cross_layers and i > 0 and self.cross_pos == 'cascade':
                ...
                if self.return_intermediate:
                    ...
                    if intermediate_dict is not None:
                        intermediate_dict['encoder_between_%d_x_%d' % (4 - i, 4 - i - 1)] = interm.clone()
            # shape = sizes[i]
            # import ipdb; ipdb.set_trace()
            for j in range(self.num_layers[i]):
                ...
                if self.return_intermediate:
                    ...
                    interm = interm.squeeze(0).permute(1, 0, 2, 3)  # T x C x H x W
                    if intermediate_dict is not None:
                        intermediate_dict['encoder_within_%d_x_%d' % (4 - i - 1, j)] = interm.clone()
                ###################################
        ...
        if self.use_cross_layers and self.cross_pos == 'post':
            ...
            outputs[1] = output1
            if intermediate_dict is not None:
                intermediate_dict['ceb_%dx%d' % (i - 1, i)] = output1.clone()
        # TODO
        # Check this later
        ...
        return outputs, intermediate_dict
...
class VOS_SwinMEDVT(nn.Module):
    ...
    def forward(self, samples: NestedTensor, return_intermediates=True):
        # import ipdb; ipdb.set_trace()
        if return_intermediates:
            intermediate_dict = OrderedDict[torch.Tensor]()
        else:
            intermediate_dict = None
        ...
        if self.training:
            ...
            return self.forward_inference(samples, intermediate_dict)

    def forward_inference(self, samples: NestedTensor, intermediate_dict=None):
        # import ipdb;ipdb.set_trace()
        ...
        with torch.no_grad():
            for stride, samples_ in samples.items():
                all_outs.append(self._forward_one_samples(samples_, intermediate_dict))
            # import ipdb;ipdb.set_trace()
    ...
    def _forward_one_samples(self, samples: NestedTensor, intermediate_dict=None):
        if not isinstance(samples, NestedTensor):
            samples = nested_tensor_from_tensor_list(samples)
        # import ipdb; ipdb.set_trace()
...
class VOS_SwinMEDVTLPROP(VOS_SwinMEDVT):
    ...
    def _forward_one_samples(self, samples: NestedTensor, intermediate_dict=None):
        ...
        if self.transformer is None:
            ...
            # import ipdb; ipdb.set_trace()
            # stage_outputs.append(features[-1].tensors.permute(1, 0, 2, 3).flatten(1).mean(1).unsqueeze(0))
            seg_feats, intermediate_dict = self.transformer(features, pos_list, batch_size, intermediate_dict)
            mask_ins = seg_feats.unsqueeze(0).permute(0, 2, 1, 3, 4)
            # import ipdb; ipdb.set_trace()
        ...
        if intermediate_dict is not None:
            intermediate_dict['task_head_out_logits'] = outputs_seg_masks_b4lprop.clone().squeeze(0).sigmoid()

        outputs_seg_masks_lprop = outputs_seg_masks_b4lprop.permute(1, 0, 2, 3)
        # import ipdb;ipdb.set_trace()
        for i in range(self.stacked):
            if self.feat_loc == 'late':
                outputs_seg_masks_lprop, lprop_stages = self.label_propagator(outputs_seg_masks_lprop, seg_feats)
            elif self.feat_loc == 'attmaps_only':
                outputs_seg_masks_lprop, lprop_stages = self.label_propagator(outputs_seg_masks_lprop,
                                                                              seg_feats[:, -8:])
        ...
        if self.lprop_mode in [1, 2]:
            ...
            if self.training:
                ...
                outputs_seg_masks = outputs_seg_masks_b4lprop*0.4+outputs_seg_masks_lprop*0.7
                # outputs_seg_masks = torch.stack([outputs_seg_masks_b4lprop, outputs_seg_masks_lprop], dim=0).mean(0)
        elif self.lprop_mode == 3:
            outputs_seg_masks = outputs_seg_masks_lprop
        # import ipdb;ipdb.set_trace()
        if intermediate_dict is not None:
            for k, v in lprop_stages.items():
                intermediate_dict[k] = v
        # import ipdb;ipdb.set_trace()
        intermediate_dict['final_logits'] = outputs_seg_masks.clone().squeeze(0).sigmoid()  # TxHxW
        # import ipdb;ipdb.set_trace()
        final_prediction = outputs_seg_masks.clone().sigmoid().squeeze(0)
        final_prediction[final_prediction >= 0.5] = 0.99
        final_prediction[final_prediction < 0.5] = 0.01
        intermediate_dict['final_prediction'] = final_prediction
        out = {
