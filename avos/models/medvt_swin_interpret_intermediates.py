    ...
    def forward(self, features, pos_list, batch_size, intermediate_dict=None):
        ...
        if self.use_decoder:
            if True:
                ...
                # import ipdb; ipdb.set_trace()
                if intermediate_dict is not None:
                    intermediate_dict['sf'] = seg_feats.clone()
        else:
            ms_feats = self.fpn(deep_feature, fpn_features)
            hr_feat = ms_feats[-1]
            seg_feats = hr_feat
        ...
        for i in range(self.num_stages):
            ...
            if self.use_cross_layers and i > 0 and self.cross_pos == 'cascade':
                ...
                if self.return_intermediate:
                    ...
                    if intermediate_dict is not None:
                        intermediate_dict['ceb_%dx%d' % (i - 1, i)] = interm.clone()
            # shape = sizes[i]
            # import ipdb; ipdb.set_trace()
            for j in range(self.num_layers[i]):
                ...
                if self.return_intermediate:
                    ...
                    interm = interm.squeeze(0).permute(1, 0, 2, 3)  # T x C x H x W
                    if intermediate_dict is not None:
                        intermediate_dict['cew_%dx%d' % (i, j)] = interm.clone()
                ###################################
...
class VOS_SwinMEDVTLPROP(VOS_SwinMEDVT):
    ...
    def _forward_one_samples(self, samples: NestedTensor, intermediate_dict=None):
        ...
        if intermediate_dict is not None:
            intermediate_dict['th_mask'] = outputs_seg_masks_b4lprop.permute(1, 0, 2, 3)
        outputs_seg_masks_lprop = outputs_seg_masks_b4lprop.permute(1, 0, 2, 3)
        # import ipdb;ipdb.set_trace()
        for i in range(self.stacked):
            if self.feat_loc == 'late':
                outputs_seg_masks_lprop, lprop_masks_enc = self.label_propagator(outputs_seg_masks_lprop, seg_feats)
            elif self.feat_loc == 'attmaps_only':
                outputs_seg_masks_lprop, lprop_masks_enc = self.label_propagator(outputs_seg_masks_lprop,
                                                                                 seg_feats[:, -8:])
        ...
        # import ipdb;ipdb.set_trace()
        if intermediate_dict is not None:
            intermediate_dict['lp_enc'] = lprop_masks_enc.squeeze(0).permute(1, 0, 2, 3)
            intermediate_dict['lp_masks'] = outputs_seg_masks_lprop.permute(1, 0, 2, 3)
        out = {
