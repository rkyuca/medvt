...
class EncATPP(nn.Module):
    def __init__(self, input_dim=384, model_dim=384, output_dim=384, num_branches=1, groups=4, dropout=0.1,
                 use_res=True, out_agg='cat'):
        super(EncATPP, self).__init__()
        self.num_branches = num_branches
        self.use_res = use_res
        self.branches = nn.ModuleList()
        self.out_agg = out_agg  # cat, add, max
        ...
        if self.num_branches > 1 and self.self.out_agg == 'cat':
    ...
    def forward(self, x):
        ...
        if self.num_branches > 1:
            if self.self.out_agg == 'cat':
                outs = torch.cat(branch_outs, dim=1)
                outs = self.out_proj(self.dropout(outs))
            elif self.self.out_agg == 'add':
                outs = torch.add(branch_outs, dim=1)
            elif self.self.out_agg == 'max':
                outs = torch.max(branch_outs, dim=1)
            else:
                raise NotImplementedError('ATPP aggr type {} not implemented'.format(str(self.out_aggr)))
        else:
            outs = branch_outs[0]
        if self.use_res:
            outs = self.shortcut(x) + outs
        return outs
...
class Transformer(nn.Module):
    ...
    def __init__(self, num_frames, backbone_dims, d_model=384, nhead=8,
        ...
        self.use_atpp = True
        if self.use_atpp:
            self.dec_atpp = DecATPP(input_dim=392, model_dim=392, output_dim=392, num_branches=3, use_res=True)
        ...
        for i in range(len(num_encoder_layers)):
            ...
            for j in range(num_encoder_layers[i]):
                ...
                encoder_layer = TransformerEncoderLayer(d_model, _nhead, _dim_feedforward, _dropout, activation,
                                                        normalize_before, _use_layer_norm, pool_kv=i > 1)
        ...
        if use_cross_layers:
            self.cross_res_layers = nn.ModuleList([
                CrossResolutionEncoderLayer(d_model=384, nhead=1, dim_feedforward=384, layer_norm=False,
                                            custom_instance_norm=True, fuse='linear' if i == 0 else None) for i in
                range(len(num_encoder_layers) - 1)
            ])
    ...
    def forward(self, features, src_key_padding_masks, pos_embeds, sizes, masks=None, pred_mems=None):
        ...
        for i in range(self.num_stages):
            ...
            if self.use_cross_layers and i > 0 and self.cross_pos == 'cascade':
                ...
                # ############################################################
                if i == 1 and src0.shape[0] != src1.shape[0]:
                    # import ipdb; ipdb.set_trace()
                ...
                # ############################################################
                # import ipdb; ipdb.set_trace()
                output = self.cross_res_layers[i - 1](src0_up, src1, mask0_up, mask1, pos0_up, pos1)
            # shape = sizes[i]
            # import ipdb; ipdb.set_trace()
...
class CrossResolutionEncoderLayer(nn.Module):
    ...
    def forward(self, feat_deep, feat_shallow, mask_deep, mask_shallow, pos_deep, pos_shallow):
        ...
        # import ipdb; ipdb.set_trace()
        attn_mask = torch.mm(mask_shallow.transpose(1, 0).double(), mask_deep.double()).bool()
        out = self.self_attn(qp, kp, value=vv, attn_mask=attn_mask, key_padding_mask=mask_deep)[0]
        out = feat_shallow + self.dropout1(out)
        out = self.norm1(out)
        out2 = self.linear2(self.dropout(self.activation(self.linear1(out))))
        out = out + self.dropout2(out2)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False, use_layer_norm=True, use_atpp=True, pool_kv=False):
        super().__init__()
        self.nhead = nhead
        ...
        if self.pool_kv:
            self.kv_pool3d = nn.AvgPool3d((3, 3, 3), stride=(2, 2, 2))
        if self.use_atpp:
            self.key_atpp = EncATPP(input_dim=384, model_dim=384, output_dim=384, num_branches=1, groups=4, dropout=0.1,
                                    use_res=True, out_agg='cat')
    ...
    def forward_post(self,
        ...
        if self.use_atpp:
            s_h, s_w = size
            t_f = src_k.shape[0] // (s_h * s_w)
            s_ch = src_k.shape[-1]
            src_k = src_k.permute(1, 2, 0).view(1, 384, t_f, s_h * s_w)
            src_k = src_k.permute(0, 2, 1, 3).reshape(1, t_f, 384, s_h, s_w).flatten(0, 1)
            src_k = src_k.permute(1, 0, 2, 3).unsqueeze(0)
            src_k = self.key_atpp(src_k)
            src_k = src_k.squeeze(0).permute(1, 0, 2, 3)
            src_k = src_k.reshape(1, t_f, s_ch, s_h, s_w).permute(0, 2, 1, 3, 4).flatten(-2)
            src_k = src_k.flatten(2).permute(2, 0, 1)
            # import ipdb;ipdb.set_trace()
            src_kv = self.key_norm(src_k)
        if self.pool_kv:
            # import ipdb; ipdb.set_trace()
            s_h, s_w = size
            t_f = src_k.shape[0] // (s_h * s_w)
            s_ch = src_k.shape[-1]
            src_k = src_k.permute(1, 2, 0).view(1, s_ch, t_f, s_h * s_w)  # 1, c, t, hw
            src_k = src_k.permute(0, 2, 1, 3).reshape(1, t_f, 384, s_h, s_w).flatten(0, 1)
            src_k = src_k.permute(1, 0, 2, 3).unsqueeze(0)
            src_k = self.kv_pool3d(src_k)
            b, c, t, h, w = src_k.shape
            src_key_padding_mask = F.interpolate(src_key_padding_mask.unsqueeze(0).reshape(1, 1, t_f, s_h, s_w).float(), size=(t, h, w), mode='nearest').bool().flatten(2).squeeze(0)
            pos_k = F.interpolate(pos_k.permute(1,2,0).reshape(1, s_ch, t_f, s_h, s_w), size=(t, h, w), mode='nearest').flatten(2).permute(2,0,1)
            src_k = src_k.squeeze(0).permute(1, 0, 2, 3)
            src_k = src_k.reshape(b, t, c, h, w).permute(0, 2, 1, 3, 4).flatten(-2)
            src_k = src_k.flatten(2).permute(2, 0, 1)
        query = self.with_pos_embed(src_q, pos_q)
        key = self.with_pos_embed(src_k, pos_k)
        value = src_k
...
class VOS_SwinMEDVTLPROP(VOS_SwinMEDVT):
    def __init__(self, args, backbone, backbone_dims, hidden_dim, transformer, num_frames,
        ...
        if feat_loc == 'early_fine':
            ...
            feat_dim = 392  # + 8  # ( 8 added for ATPP)
            hidden_dim = 128
