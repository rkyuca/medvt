...
class LabelPropagator(nn.Module):
    ...
    def forward(self, masks, feats):
        ...
        # import ipdb;ipdb.set_trace()
        # stage_outs.append(masks.squeeze(0).permute(1, 0, 2, 3))
        feats = F.interpolate(feats, masks_enc.shape[-2:])
        stage_outs['lpp_masks_enc_in'] = masks_enc.clone().squeeze(0)
        # stage_outs.append(torch.cat((masks_enc.squeeze(0), feats), dim=1))
        ...
        # import ipdb; ipdb.set_trace()
        # stage_outs.append(new_masks_enc.squeeze(0).permute(1, 0, 2, 3))
        stage_outs['lpp_masks_enc_out'] = new_masks_enc.clone().squeeze(0).permute(1, 0, 2, 3)
        stage_outs['lpp_out_logits'] = masks.clone().squeeze(0).squeeze(0).sigmoid()

        # stage_outs.append(torch.cat((new_masks_enc.squeeze(0).permute(1, 0, 2, 3), masks_enc.squeeze(0), feats.squeeze(0)), dim=1))
        # stage_outs.append( torch.cat((new_masks_enc,masks),dim=1).squeeze(0).permute(1, 0, 2, 3))
        # stage_outs.append(masks.squeeze(0).permute(1, 0, 2, 3))

        # import ipdb;ipdb.set_trace()
        masks = masks.squeeze(0)
        return masks.squeeze(1), stage_outs
