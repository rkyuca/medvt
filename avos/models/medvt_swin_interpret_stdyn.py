...
from avos.models.label_propagation_stdyn import LabelPropagator
...
class Transformer(nn.Module):
    ...
    def forward(self, features, pos_list, batch_size):
        ...
        # import ipdb; ipdb.set_trace()
        if self.use_decoder:
            if self.query_decoder_scales > 1:
                ms_feats = self.fpn(deep_feature, fpn_features)
                # import ipdb; ipdb.set_trace()
                # fpn_intermediates = [fi.clone() for fi in ms_feats]
                # trans_stage_outputs = trans_stage_outputs + fpn_intermediates
                ...
                # import ipdb; ipdb.set_trace()
                # trans_stage_outputs.append(obj_attn_masks.permute(1, 0, 2, 3).flatten(1).permute(1, 0).mean(dim=0, keepdim=True))
                # trans_stage_outputs.append(obj_attn_masks.permute(1, 0, 2, 3).flatten(1).permute(1, 0).std(dim=0, keepdim=True))
                # trans_stage_outputs.append(obj_attn_masks.permute(1, 0, 2, 3).flatten(1).permute(1, 0).norm(dim=0, keepdim=True))
                #  trans_stage_outputs.append(hr_feat.permute(1, 0, 2, 3).flatten(1).mean(1).unsqueeze(0))
                # import ipdb; ipdb.set_trace()
                if self.decoder_attn_fuse is not None and self.decoder_attn_fuse == 'add':
                ...
                # import ipdb; ipdb.set_trace()
                # trans_stage_outputs.append(seg_feats.clone())
                # trans_stage_outputs.append(seg_feats.permute(1, 0, 2, 3).flatten(1).permute(1, 0).std(dim=0, keepdim=True))
                # trans_stage_outputs.append(seg_feats.permute(1, 0, 2, 3).flatten(1).permute(1, 0).norm(dim=0, keepdim=True))
            else:
                ####################################
                dec_features = []
                pos_embed_list = []
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, features, src_key_padding_masks, pos_embeds, sizes, masks=None, pred_mems=None):
        ...
        if self.use_cross_layers and self.cross_pos == 'post':
            ...
            # import ipdb;ipdb.set_trace()
            outputs[1] = output1
            all_intermediates.append(output1.clone())
        # TODO
        # Check this later
...
class VOS_SwinMEDVT(nn.Module):
    def __init__(self, args, backbone, backbone_dims, hidden_dim, transformer, num_frames, temporal_strides=[1],
        ...
        if transformer is not None and transformer.use_decoder and transformer.query_decoder_scales > 1:
...
class VOS_SwinMEDVTLPROP(VOS_SwinMEDVT):
    def __init__(self, args, backbone, backbone_dims, hidden_dim, transformer, num_frames,
        ...
        self.aux_loss = hasattr(args, 'aux_loss') and args.aux_loss > 0
    ...
    def _forward_one_samples(self, samples: NestedTensor, stages=[]):
        ...
        if self.transformer is None:
            ...
            #    seg_feats.permute(1, 0, 2, 3).flatten(1).permute(1, 0).mean(dim=0).unsqueeze(0).clone().detach())
            mask_ins = seg_feats.unsqueeze(0).permute(0, 2, 1, 3, 4)
        ...
        if self.lprop_mode in [1, 2]:
            ...
            outputs_seg_masks = outputs_seg_masks_lprop
        # import ipdb;ipdb.set_trace()
        stage_outputs = stage_outputs+lprop_stages
        # stage_outputs.append(lprop_masks_enc.squeeze(0).permute(1, 0, 2, 3))
        # import ipdb; ipdb.set_trace()
        # use_masks=False
        # seg = outputs_seg_masks.clone()
        #  _,_, seg_h, seg_w = seg.shape
        # import ipdb;ipdb.set_trace()
        # stage_outputs.append(lprop_masks_enc.squeeze(0).flatten(1).permute(1, 0).mean(dim=0, keepdim=True))
        # stage_outputs.append(lprop_masks_enc.squeeze(0).flatten(1).permute(1, 0).std(dim=0, keepdim=True))
        # stage_outputs.append(lprop_masks_enc.squeeze(0).flatten(1).permute(1, 0).norm(dim=0, keepdim=True))
