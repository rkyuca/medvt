...
class Pred_endecoder(nn.Module):
    ...
    def __init__(self, config, args, channel=384, vis_dim=[64, 128, 320, 512]):
        ...
        self.d_model = channel
        self.visual_backbone = args.visual_backbone
        self.mm_context = args.mm_context
        ...
        if args.mm_context == 'bidir':
            self.mm_context_encoder = MMContextEncoderBiDirCrossAttn(config, args, self.vis_dim, channel=channel, context_channel=128)
        elif args.mm_context == 'tpavi':
            self.mm_context_encoder = MMContextEncoderTPAVI(config, args, self.vis_dim, channel=channel)
        else:
            self.mm_context_encoder = MMContextEncoderTPAVI(config, args, self.vis_dim, channel=channel)
        ...
        if self.training:
            ...
            if self.visual_backbone == 'swinB':
                ...
                self.initialize_pvt_weights(args.pvtv2_pretrained_path)
    ...
    def initialize_pvt_weights(self, pvtv2_pretrained_path):
        pvt_model_dict = self.encoder_backbone.state_dict()
        pretrained_state_dicts = torch.load(pvtv2_pretrained_path)
