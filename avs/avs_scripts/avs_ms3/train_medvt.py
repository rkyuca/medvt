...
from avs.avs_scripts.avs_ms3.torchvggish import vggish
...
if __name__ == "__main__":
    ...
    parser.add_argument("--context_channel", default=128, type=int)
    ...
    # TPAVI
    parser.add_argument('--masked_av_flag', action='store_true', default=False,
                        help='additional sa/masked_va loss for five frames')
    parser.add_argument("--lambda_1", default=0.1, type=float, help='weight for balancing l4 loss')
    ...
    """
    parser.add_argument("--qdec_dim_ff", default=2048, type=int,
                        help='query decoder dim_ff')
    parser.add_argument("--qdec_nhead", default=8, type=int,
                        help='query decoder layers')
    """
 
    # Optimizer 
    parser.add_argument("--session_name", default="ms3_pvt_medvt", type=str, help="the MS3 setting")
    parser.add_argument("--train_im_size", default=None, type=int)
    parser.add_argument("--val_im_size", default=None, type=int)
    ...
    parser.add_argument('--log_dir', default='/local/riemann/home/rezaul/outputs/avsbench/avs_ms3/train_logs', type=str)
    ...
    scripts_to_save = ['train_medvt.py', 'test_medvt.py', 'config.py', 'dataloader.py', 'loss.py', './model/MEDVT_AVSModel.py']
    ...
    # import ipdb;ipdb.set_trace()
    # Data
    train_dataset = MS3Dataset('train', resize_to=args.train_im_size)
    ...
    val_dataset = MS3Dataset('val',resize_to=args.val_im_size)
    ...
    for epoch in range(args.max_epoches):
        ...
        for n_iter, batch_data in enumerate(train_dataloader):
            ...
            # import ipdb;ipdb.set_trace()
            output, v_map_list, a_fea_list = model(imgs, audio_feature, B, frame)  # [bs*5, 1, 224, 224]
            ...
            if (global_step - 1) % 70 == 0:
                train_log = 'Iter:%5d/%5d, Total_Loss:%.4f, iou_loss:%.4f, sa_loss:%.4f, lr: %.4f-%0.5f' % (
        ...
        with torch.no_grad():
            for n_iter, batch_data in enumerate(val_dataloader):
                ...
                output, _, _ = model(imgs, audio_feature, B, frame)  # [bs*5, 1, 224, 224]
